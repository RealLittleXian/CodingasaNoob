...
// 对记录进行排序的函数
int sortRecordsByName(Record *records, int *pnumUsers)
{
    if (*pnumUsers == 0)
    {
        return 1;
...
// 对记录进行排序的函数
int sortRecordsByID(Record *records, int *pnumUsers)
{
    if (*pnumUsers == 0)
    {
        return 1;
...
// 将用户记录写入文件
void writeToFile(Record *records, int *pnumUsers)
{
...
// 从文件中读取用户记录并显示
void readFromFile(int *pnumUsers)
{
    char name[MAX_FILENAME_SIZE];
    printf("Enter file name: ");
    getchar();
    ...
    while (fscanf(file, "%s %s %d %d", records[numUsers].id, records[numUsers].name, &records[numUsers].income, &records[numUsers].expense) == 4)
    {
        numUsers++;
    }
    *pnumUsers=numUsers;
...
int main()
{
    ...
    do
    {
        ...
        case 1:
            ...
            break;
        case 2:
            sortRecordsByName(records, &numUsers);
        ...
        case 5:
            ...
            break;
        case 6:
            sortRecordsByID(records, &numUsers);
            listAllRecords(records, &numUsers);
            break;
        case 7:
            writeToFile(records, &numUsers);
            break;
        case 8:
            readFromFile(&numUsers);
            break;
        case 0:
